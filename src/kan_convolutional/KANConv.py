...
class KAN_Convolutional_Layer(torch.nn.Module):
    ...
    def plot_b_splines(self, n):
        """
        Plot the n-th B-spline basis function from the first KAN_Convolution layer.

        Args:
            n (int): Index of the B-spline basis function to plot.
        """
        if len(self.convs) > 0:
            self.convs[0].plot_b_splines(n)
        else:
            raise ValueError("No KAN_Convolution layers found.")
